...
namespace Vaquinha.AutomatedUITests
{
                                               ...
                                               IClassFixture<CartaoCreditoFixture>
	{
		...
		[Fact]
		public void DoadoresUI_AcessoTelaDoadores()
		{
			//Arrange
			_driverFactory.NavigateToUrl("https://localhost:44317/");
			_driver = _driverFactory.GetWebDriver();
		...
		[Fact]
		public void DoacaoUI_AcessoTelaHome_Instituicoes()
		{
			//Arrange
			_driverFactory.NavigateToUrl("https://localhost:44317/");
			_driver = _driverFactory.GetWebDriver();
			...
			// Act
			IWebElement webElement = null;
			webElement = _driver.FindElement(By.Id("carouselInstituicoes"));

			// Assert
			webElement.Displayed.Should().BeTrue(because: "instituições exibidas");
		}
	}
}