...
namespace Vaquinha.Unit.Tests.DomainTests
{
    ...
    public class CartaoCreditoTests: IClassFixture<CartaoCreditoFixture>
    {
        ...
        public void CartaoCredito_NumeroValidadeCVVInvalido_CartaoCreditoInvalido()
        {
            ...
            cartaoCredito.ErrorMessages.Should().Contain("Campo Número de cartão de crédito inválido", because: "o campo Número de cartão de crédito é inválido.");
            cartaoCredito.ErrorMessages.Should().Contain("Campo CVV inválido", because: "o campo está CVV inválido.");
            cartaoCredito.ErrorMessages.Should().Contain("Campo Data de Vencimento do cartão de crédito inválido", because: "o campo Data de Vencimento do cartão crédito está inválido.");
        }
        ...
        [Fact]
        [Trait("CartaoCredito", "CartaoCredito_CartaoCredito_Validade_CVV_Invalidos")]
        public void CartaoCredito_CartaoCredito_Validade_CVV_Invalidos()
        {
            // Arrange
            var cartaoCredito = _fixture.CartaoCredito_Validade_CVV_Invalidos();
            ...
            // Assert
            valido.Should().BeFalse(because: "deve possuir erros de validação");
            cartaoCredito.ErrorMessages.Should().HaveCount(2, because: "2 campos obrigatórios estão incorretos.");

            cartaoCredito.ErrorMessages.Should().Contain("Campo CVV inválido", because: "Campo CVV inválido.");
            cartaoCredito.ErrorMessages.Should().Contain("Campo Data de Vencimento do cartão de crédito inválido", because: "Campo Data de Vencimento do cartão de crédito inválido.");
        }
