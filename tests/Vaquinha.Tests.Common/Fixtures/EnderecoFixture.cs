...
namespace Vaquinha.Tests.Common.Fixtures
{
    ...
    public class EnderecoFixture
    {
        ...
        public Endereco EnderecoNumeroInvalido()
        {
            ...
            faker.CustomInstantiator(f =>
                new Endereco(Guid.NewGuid(), "14800-000", endereco.StreetAddress(false), string.Empty, endereco.City(), endereco.StateAbbr(), "16995811385", "1000000A"));

            return faker.Generate();
        }
        public Endereco EnderecoNumeroVazio()
        {
            var endereco = new Faker("pt_BR").Address;
